...
export function Avatar(props) {
  ...
  useEffect(() => {
    ...
    const validAnimation = animations.find((a) => a.name === message.animation);
    if (validAnimation) {
      setAnimation(message.animation);
    } else {
      console.warn(`Animation "${message.animation}" not found, using "Idle" instead`);
      setAnimation("Idle");
    }
  ...
  const [animation, setAnimation] = useState(() => {
    if (!animations || animations.length === 0) {
      return "Idle"; // fallback if no animations loaded yet
    }
